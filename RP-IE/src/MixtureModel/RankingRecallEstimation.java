...
public class RankingRecallEstimation {
	 ...
	 */
	public static void main(String[] args) throws IOException, ClassNotFoundException, ParseException, InvalidVectorIndexException {
		...
		List<String> secondSample = new ArrayList<String>(sortedCollection.subList(0, 1000));
		RankingOutlierDetector outlierDetector = new ScoreBasedOutliersDetector(scoresCollection,0.2);
		int numWellModeled = 0;
		for(String doc : collection){
		...
		for(int i=0; i<secondSample.size(); i++){
			String doc = secondSample.get(i);
			if(outlierDetector.isOutlier(doc)){
				secondSample.remove(i);
			}
		}
		System.out.println("The second sample contains " + secondSample.size() + " documents.");
		
		double fracDistribution = (double)numWellModeled/(double)collection.size();
		RankingRecallEstimator estimator = new RankingRecallEstimator(scoresCollection, usefulSample, outlierDetector, new ExplicitSampleParameterEstimator(secondSample,fracUseful,fracDistribution));		

...
public class DocumentScoresAnalysis {
	public static void main(String[] args) throws IOException, ParseException, ClassNotFoundException, InvalidVectorIndexException, WriteException {
		...
		for(int split : new int[]{1,2}){
			for(String relationship : new String[]{"Indictment-Arrest-Trial", "VotingResult", "PersonCareer"}){
				...
				saveWeightVector(weightVector, relationship, split, workbook, currentSheet++);
				...
				saveDocumentScores(sortedCollection, scoresCollection, relationship, split, workbook, currentSheet++,10000);
				
				System.out.println("Performing Chi-Square test");
				double significance = 0.05;
				DistributionFactory factory = new GaussianDistributionFactory<Float>();
				ChiSquareTestExecutor<String,Float> test = new ChiSquareTestExecutor<String,Float>(10, 5, significance, factory);
				System.out.println("Null hypothesis: The scores of the documents follow a " + factory.getDistributionName());
				boolean testResult = test.performFitTest(scoresCollection);
				if(testResult){
					System.out.println("Result: We have strong evidence (more than " + (100*(1-significance) + "% confidence) that we can reject the Null Hypothesis"));
					System.out.println("Thus, we believe that the scores of the documents do not follow a " + factory.getDistributionName());
				}else{
					System.out.println("Result: We do not have strong evidence (more than " + (100*(1-significance) + "% confidence) to reject the Null Hypothesis"));
					System.out.println("Thus, we believe that the scores of the documents may follow a " + factory.getDistributionName());
				}
				
				factory = new GaussianDistributionFactory<Double>();
				ChiSquareTestExecutor<Long,Double> testFeatures = new ChiSquareTestExecutor<Long,Double>(1000, 5, significance, factory);
				System.out.println("Null hypothesis: The weights of the features follow a " + factory.getDistributionName());
				testResult = testFeatures.performFitTest(weightVector);
				if(testResult){
					System.out.println("Result: We have strong evidence (more than " + (100*(1-significance) + "% confidence) that we can reject the Null Hypothesis"));
					System.out.println("Thus, we believe that the weights of the features do not follow a " + factory.getDistributionName());
				}else{
					System.out.println("Result: We do not have strong evidence (more than " + (100*(1-significance) + "% confidence) to reject the Null Hypothesis"));
					System.out.println("Thus, we believe that the weights of the features may follow a " + factory.getDistributionName());
				}
			}
		}
	...
	private static void saveDocumentScores(List<String> sortedCollection,
			Map<String, Float> scoresCollection, String relationship,
			int split, WritableWorkbook workbook, int currentSheet,
			int maxNumDocs) throws RowsExceededException, WriteException {
		workbook.createSheet(relationship+"-"+split+"-S", currentSheet);
		WritableSheet excelSheet = workbook.getSheet(currentSheet);
		addLabel(excelSheet, 0, 0, "Document");
		addLabel(excelSheet, 0, 1, "Score");

		if(sortedCollection.size()>maxNumDocs){
			double v = (double)sortedCollection.size()/(double)maxNumDocs;

			List<String> newCollection = new ArrayList<String>();
			double currentEl=0;
			while(currentEl<sortedCollection.size()){
				int index = (int) Math.round(currentEl);
				newCollection.add(sortedCollection.get(index));
				currentEl+=v;
			}
		...
		for(int i=0; i<sortedCollection.size(); i++){
			String doc = sortedCollection.get(i);
			addLabel(excelSheet, i+1, 0, doc);
			addNumber(excelSheet, i+1, 1, scoresCollection.get(doc));
		}		
	}

	private static void saveWeightVector(Map<Long, Double> weightVector,
			String relationship, int split, WritableWorkbook workbook,int currentSheet) throws IOException, WriteException {

		workbook.createSheet(relationship+"-"+split+"-F", currentSheet);
		WritableSheet excelSheet = workbook.getSheet(currentSheet);

		addLabel(excelSheet, 0, 0, "Feature");
		addLabel(excelSheet, 0, 1, "Value");

		final Map<Long, Double> weightVectorFinal = weightVector;

		List<Long> collection = new ArrayList<Long>(weightVector.keySet());
		Collections.sort(collection, new Comparator<Long>() {
			@Override
			public int compare(Long doc1, Long doc2) {
				Double score2 = weightVectorFinal.get(doc2);
				if(score2==null){
					score2=0.0;
				}
				Double score1 = weightVectorFinal.get(doc1);
				if(score1==null){
					score1=0.0;
				}
		...
		for(int i=0; i<collection.size(); i++){
			Long l = collection.get(i);
			addNumber(excelSheet, i+1, 0, l);
			addNumber(excelSheet, i+1, 1, weightVector.get(l));
		}
	}

	private static void addNumber(WritableSheet sheet, int row, int column,
	...
	private static void addLabel(WritableSheet sheet, int row, int column, String s)

...
public class FindThetaParameter {
...
=======
		String[] relationships = new String[]{"OrgAff","Outbreaks","PersonCareer","NaturalDisaster","ManMadeDisaster","Indictment-Arrest-Trial","VotingResult"};
>>>>>>> .r7
		...
		for(String relationship : relationships){
			for(int split : splits){
				//double thetaRSVM= getThetaScoreRSVM(conn, new HashSet<String>(collectionFixed), split, relationship, root, numQueries, extractor, subPaths, folderDesign, path);
				double thetaQBC= getThetaScoreQBC(conn, new HashSet<String>(collectionFixed), split, relationship, root, numQueries, extractor, subPaths, folderDesign, path,
						comb, independent, numEpochs, reg, lambda, termsAreQueries);
				
<<<<<<< .mine
				System.out.println(relationship + "," + split + ",RSVM," + thetaRSVM);
				System.out.println(relationship + "," + split + ",QBC," + thetaQBC);
=======
				//System.out.println(relationship + "|" + split + "|RSVM|" + thetaRSVM);
				System.out.println(relationship + "|" + split + "|QBC|" + thetaQBC);
>>>>>>> .r7
	...
	private static double getThetaScoreRSVM(IndexConnector conn, Set<String> collection, int split,
		...
		ThetaParameterFinder finder = new ThetaParameterFinder((1-frac), frac, 1000);
		return finder.findTheta(sample, usefulSample, scoresCollection);
	}

	private static double getThetaScoreQBC(IndexConnector conn, Set<String> collection, int split,
...
/*		if(relationship.equals("OrgAff") || relationship.equals("Outbreaks")){
		...
		Map<String,Double> scoresCollection = model.getScores(new HashSet<String>(sample),conn,independent);
		...
		ThetaParameterFinder finder = new ThetaParameterFinder((1-frac), frac, 1000);
		return finder.findTheta(sample, usefulSample, result);
	}
}
